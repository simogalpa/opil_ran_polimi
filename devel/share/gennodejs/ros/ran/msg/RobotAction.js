...
class RobotAction {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.attributes.forEach((val) => {
      bufferOffset = Tuple.serialize(val, buffer, bufferOffset);
    });
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.attributes[i] = Tuple.deserialize(buffer, bufferOffset)
    }
  ...
  static getMessageSize(object) {
    ...
    object.attributes.forEach((val) => {
      length += Tuple.getMessageSize(val);
    });
  ...
  static Resolve(msg) {
    ...
    if (msg.attributes !== undefined) {
      ...
      for (let i = 0; i < resolved.attributes.length; ++i) {
        resolved.attributes[i] = Tuple.Resolve(msg.attributes[i]);
      }
    }
    else {
      resolved.attributes = []
